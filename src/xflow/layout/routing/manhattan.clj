  ...
  (let [vertical? (not= mode "horizontal")
        ...
        check (fn [val]
                (let [segment (if vertical?
                                {:x1 val :x2 val :y1 seg-min :y2 seg-max}
                                {:x1 seg-min :x2 seg-max :y1 val :y2 val})]
                  (some #(intersects-node? segment %) nodes)))]
    (loop [offset 0]
      (if (> offset 200)
        start-val
        (let [try-pos (+ start-val (* offset (/ increment 20)))]
...
(defn- path-valid? [points nodes]
  (every? (fn [[p1 p2]]
          ...
          (partition 2 1 (geo/simplify-points points))))
                           ...
                           (let [p1-side (get-port-side n1 p1)
                                 ...
                                 points (if vertical?
                                          ...
                                          [p1
                                           ...
                                           p2-dock
                                           p2])]
                             (assoc e :points (geo/simplify-points points) :p1 p1 :p2 p2 :strategy :side-loop :side side))
                           :else
                           (let [points (route-segment p1 p2 nodes mode)]
                             (assoc e :points (geo/simplify-points points) :p1 p1 :p2 p2 :strategy strategy))))
                       e)))
                 edges))))
